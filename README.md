...
Windows 마스터 또는 앱별 음소거 상태가 바뀌면 주 모니터 오른쪽 위에 약 1.5초 동안 작은 상태 오버레이를 표시합니다. 오버레이는 마우스 클릭과 키보드 포커스를 가로채지 않으며, 메인 화면의 `오버레이` 버튼으로 바로 끄거나 다시 켤 수 있습니다. 설정은 `%LocalAppData%\MutePilot\settings.json`의 `overlayEnabled`에 저장되고, 기존 설정 파일에 이 항목이 없으면 기본값인 ON으로 시작합니다.

오버레이는 MutePilot이 별도의 WPF 창으로 표시하며 게임 프로세스에 접근하거나 화면에 코드를 주입하지 않습니다. 일반 창과 테두리 없는 창에서는 표시할 수 있지만, 독점 전체 화면 애플리케이션 위에는 보이지 않을 수 있습니다. 자동 숨김, 표시 시간 재시작, 클릭 통과, 포커스 유지, 설정 저장·복원은 실제 Windows 환경에서 검증했습니다. SuddenAttack을 실제로 전면에 둔 상태의 표시는 추가 수동 확인이 필요합니다.

**음소거 상태 오버레이: 구현 완료 / Windows 창 동작 검증 완료 / SuddenAttack 수동 검증 필요**
...
* 선택형 음소거 상태 오버레이 — 구현 완료, 일반 Windows 환경 검증 완료
...
8. 선택형 음소거 상태 오버레이 구현 — 완료, SuddenAttack 전면 표시 수동 검증 필요
9. 시스템 트레이와 시작 옵션 검토
