...
활성 애플리케이션 오디오 세션 조회와 앱별 음소거 토글 코드를 구현했습니다. 실제 PC에서 `suddenattack`, `whale` 세션이 PID와 함께 표시되는 것까지 확인했으며, 앱별 음소거 전환은 사용자 환경에서 직접 검증해야 합니다. 전역 단축키는 아직 구현하지 않았습니다.

**앱별 음소거: 구현 완료 / 실제 사용자 환경 검증 필요**

## 앞으로 구현할 기능

* Windows 마스터 음소거/음소거 해제 — 구현 및 실제 동작 수동 검증 완료
* 활성 Windows 오디오 세션을 사용하는 애플리케이션 감지 — 구현 및 실제 세션 조회 확인 완료
* 애플리케이션별 음소거/음소거 해제 — 구현 완료, 실제 동작 수동 검증 필요
...
4. 애플리케이션별 음소거 토글 구현 — 완료, 실제 동작 수동 검증 필요
5. 사용자 지정 전역 단축키 추가
6. 단축키 바인딩과 설정 저장
7. UI, 예외 처리, 배포 문서 정리
