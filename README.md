...
메인 화면을 네이비·차콜 기반의 사이드바 대시보드로 다시 구성했습니다. `홈 / 대시보드`, `애플리케이션`, `실행 설정`, `정보` 영역으로 이동할 수 있고 Master Audio, 앱별 오디오, 오버레이, 실행 권한과 버전 정보를 한 창에서 확인할 수 있습니다.

화면 테마는 `기본 버전 (다크모드)`, `화이트모드`, `핑크모드` 세 가지입니다. 색상은 WPF `ResourceDictionary`로 분리했으며 선택 즉시 화면에 반영되고 `%LocalAppData%\MutePilot\settings.json`에 저장됩니다. 기존 설정 파일에 `theme`이 없으면 다크모드로 시작하며 파일을 자동으로 다시 쓰지 않습니다.
